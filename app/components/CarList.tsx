  ...
  return (
    <section>
      ...
      <div className="movie-list grid grid-flow-row justify-items-center gap-6 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 mb-16">
        {cars.map((car) => {
          return <CarItem key={`car-${car.nid}`} {...car} listTitle={listTitle} />;
        })}
      </div>
      {numberOfPages && currentPage < numberOfPages &&
        ...
        >
          {isLoading ? 
            <div role="status">
              <svg aria-hidden="true" className="h-8 text-gray-200 animate-spin text-gray fill-gray-2 w-full" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
