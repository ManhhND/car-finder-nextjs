      ...
      <li className="list-none bg-white p-6 w-full">
        <div className="headline-section flex items-center">
          ...
          <div className="favorite w-1/12">
            <input type="checkbox" id={`${listTitle} ${title}`} onChange={handleFavorite} className="hidden" />
            <label htmlFor={`${listTitle} ${title}`} className="cursor-pointer">
              <Image src={favorite ? heartSVG : outlinedHeartSVG} alt="favorite" />
            </label>
          </div>
        </div>
        ...
        <div className="action-section flex justify-between items-center">
            <div className="font-bold text-xl">{field_price}/ <span className="text-gray-5 font-normal text-base">day</span></div>
            <Link href={`/car/${nid}`} className="text-white bg-blue py-3 px-6 rounded">
              Rent Now
            </Link>
          </div>
      </li>
